...
import static com.company.NCL.NCL.Gender.MAN;
import static com.company.NCL.NCL.Gender.WOMAN;
...
public abstract class NCLNameCaseCore extends NCL {
    ...
    private NCLNameCaseCore fullReset() {
        ...
        this.index.put(SECOND_NAME, new ArrayList<>());
        this.reset();
        this.notReady();
        return this;
    }
    ...
    protected String last(int length, int stopAfter) {
        ...
        if (!(this.workindLastCache.containsKey(length) && this.workindLastCache.get(length).containsKey(stopAfter))) {
            ...
            String substr = substring(this.workingWord, -length, cut);
    ...
    public NCLNameCaseCore setFirstName(String firstname) {
        if (isNotEmpty(firstname)) {
            ...
            nameCaseWord.setNamePart(FIRST_NAME);
            this.notReady();
        }
        return this;
    }
    ...
    public NCLNameCaseCore setLastName(String secondname) {
        if (isNotEmpty(secondname)) {
            ...
            this.words.add(nameCaseWord);
            nameCaseWord.setNamePart(SECOND_NAME);
            this.notReady();
        }
        return this;
    }
    ...
    public NCLNameCaseCore setFatherName(String fathername) {
        if (isNotEmpty(fathername)) {
            ...
            this.words.add(nameCaseWord);
            nameCaseWord.setNamePart(FATHER_NAME);
            this.notReady();
        }
        return this;
    }
     ...
     * return NCLNameCaseCore
     */
    public NCLNameCaseCore convertToOtherCases(String secondName, String firstName, String fatherName) {
    ...
    private void prepareGender(NCLNameCaseWord word) {
        if (!word.isGenderSolved()) {
            ...
            switch (namePart) {
                case FIRST_NAME -> this.genderByFirstName(word);
                case FATHER_NAME -> this.genderByFatherName(word);
                case SECOND_NAME -> this.genderBySecondName(word);
            }
        }
    }
        ...
        if (!this.words.isEmpty()) {
            ...
            if (n >= 0) {
                if (this.words.size() > n) {
                    ...
                    double min = genders.values().stream().min(Double::compareTo).get();
                    double max = genders.values().stream().max(Double::compareTo).get();
                    //Вероятность автоопределения пола [0..10]. Достаточно точно при 0.1
                    double genderKoef = max - min;
                    return word.gender();
                }
            }
        }
    ...
    private void wordCase(NCLNameCaseWord word) {
        ...
        Supplier<Boolean> method;

        NamePart namePartLetter = word.getNamePart();
        method = switch (namePartLetter) {
            case FATHER_NAME -> word.gender() == MAN ? this::manFatherName : this::womanFatherName;
            case FIRST_NAME -> word.gender() == MAN ? this::manFirstName : this::womanFirstName;
            case SECOND_NAME -> word.gender() == MAN ? this::manSecondName : this::womanSecondName;
        };
    ...
    private List<String> getCasesConnected(List<Integer> indexArray, int number) {
        List<String> readyArr = makeListOfStrings();
    ...
    private List<String> getCasesConnected(List<Integer> indexArray) {
        List<List<String>> readyArr = makeListOfStrings();
        for (int index : indexArray) {
            readyArr.add(this.getWordCase(this.words.get(index)));
        }

        //Масив нужно скелить каждый падеж
        List<String> resultArr = makeListOfStrings();
        for (int c = 0; c < this.getCaseCount(); c++) {
            List<String> tmp = makeListOfStrings();
            for (List<String> strings : readyArr) {
                tmp.add(strings.get(c));
            }
    ...
    public List<String> getFirstNameCase() {
        this.allWordCases();
        return this.getCasesConnected(this.index.get(FIRST_NAME));
    }
    ...
    public List<String> getSecondNameCase() {
        this.allWordCases();
        return this.getCasesConnected(this.index.get(SECOND_NAME));
    }
    ...
    public List<String> getFatherNameCase() {
        this.allWordCases();
        return this.getCasesConnected(this.index.get(FATHER_NAME));
    }
     ...
     */
    public String convertFirstNameToOtherCase(String firstName, Cases cases, Gender gender) {
        this.fullReset();
        this.setFirstName(firstName);
        this.setGender(gender);
        return this.getFirstNameCase().get(getCaseIndex(cases));
    }
     ...
     * return mixed массив или строка с нужным падежом
     */
    public String convertSecondNameToOtherCase(String secondName, Cases cases, Gender gender) {
        this.fullReset();
        this.setLastName(secondName);
        this.setGender(gender);
        return this.getSecondNameCase().get(getCaseIndex(cases));
    }

    /**
     * В система заносится сразу фамилия, имя, отчество в Формате одной переменной
     * param fullName фамилия имя отчетсво
     * return NCLNameCaseCore
     */
    public String convertToOtherCase(String fullName, Cases cases) {
        this.fullReset();
        this.setFirstName(fullName);
        this.setFatherName(fullName);
        this.setGender(Gender.valueOf(String.valueOf(genderDetect(fullName))));
        return this.getSecondNameCase().get(getCaseIndex(cases)) + " "
                + this.getFirstNameCase().get(getCaseIndex(cases)) + " "
                + this.getFatherNameCase().get(getCaseIndex(cases));
    }

    public int getCaseIndex(Cases cases) {
        switch (cases) {
            case NAZIVNIY -> {
                return 0;
            }
     ...
     * return mixed массив или строка с нужным падежом
     */
    public String qFatherName(String fatherName, Cases cases, Gender gender) {
        this.fullReset();
        this.setFatherName(fatherName);
        this.setGender(gender);
        return this.getFatherNameCase().get(getCaseIndex(cases));
    }
